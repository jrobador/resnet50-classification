...
def synth_input_fn(is_training, batch_size, height=224, width=224,
        num_channels=3, num_classes=3, dtype=tf.float32):
  ...
  data = data.prefetch(buffer_size=6)
  return data
...
def get_filenames(is_training, data_dir):
  ...
  if is_training:
    return [
        os.path.join(data_dir, 'train-%05d-of-00007' % i)
        for i in range(NUM_TRAIN_FILES)]
  else:
    return [
        os.path.join(data_dir, 'test-%05d-of-00002' % i)
        for i in range(1)]
...
def parse_record(raw_record, is_training, dtype, label_offset=1):
  ...
  """
  image_buffer, label, bbox = _parse_example_proto(raw_record) #, filename
