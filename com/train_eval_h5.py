...
flags.DEFINE_string(
    'model', './float_model/resnet_50.h5',
    'TensorFlow \'GraphDef\' file to load.')
flags.DEFINE_bool(
    'eval_tfrecords', True,
    'If True then use tf_records data .')
flags.DEFINE_string(
    'data_dir', '/workspace/resnet50-classification/bugdetection-classification/scripts/tf_records/bugdetection/TF-Records500',
...
flags.DEFINE_string(
    ...
    'The directory where put the eval images')
flags.DEFINE_string(
    'eval_image_list',  '/workspace/resnet50-classification/bugdetection-classification/model_archives/validation/val_labels.txt', 'file has validation images list')
flags.DEFINE_string(
    'save_path', "train_dir_final2",
...
flags.DEFINE_integer('batch_size', 2, 'Train batch size')
flags.DEFINE_integer('epochs', 2, 'Train epochs')
flags.DEFINE_integer('eval_batch_size', 15, 'Evaluate batch size')
...
def main():
  ...
  elif FLAGS.createnewmodel:                                            #PARTE NUEVA
      #for training the model from scratch use the following:
      basemodel = ResNet50(weights='imagenet', include_top=True,input_tensor=Input(shape=(224, 224, 3)))        #CAMBIAR SIZE HERE
      base_output = basemodel.layers[175].output 
      new_output = tf.keras.layers.Dense(activation="softmax", units=3)(base_output)      #CAMBIAR CANTDAD DE CLASES
