...
void runResnet50(vart::Runner* runner_ori, const string imagePath) {
  ...
  for (unsigned int n = 0; n < images.size(); n += batchSize) {
        ...
        for (int w = 0; w < inWidth; w++) {
          for (int c = 0; c < 3; c++) {
            ...
            imageInputs[i * inSize + h * inWidth * 3 + w * 3 + c] =
                ((float)(image2.at<Vec3b>(h, w)[c] - mean[c]));
    ...
    /*run*/
        /* ESTA LINEA GENERA EL ERROR */
    auto job_id = runner->execute_async(inputsPtr, outputsPtr);
        /* ESTA LINEA GENERA EL ERROR */ 
    cout << "El código llega hasta acá"<< endl;
...
int main(int argc, char* argv[]) {
  // Check args
  if (argc != 3) {
    cout << "Usage of resnet50 demo: ./resnet_BC [model_file] [image_dir]" << endl;
    return -1;
  }
